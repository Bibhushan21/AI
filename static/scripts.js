...
async function fetchDataProgressively(query) {
    ...
    try {
        const dataSource = document.getElementById('data-source').value; // Get selected data source
        ...
        const rawDataResponse = await fetch('/mcp/fetch', {
            ...
            body: JSON.stringify({ query, fetch_only: true, data_source: dataSource }) // Include data source
        });
...
queryForm.addEventListener('submit', async function(event) {
    ...
    const timeoutId = setTimeout(() => {
        document.getElementById('rawData').innerHTML = 
            '<div class="info-message">Request is taking longer than usual. Please wait... \n If the request is taking too long and AI analysis is also displayed, press "Submit" button again.</div>';
    }, 5000);
