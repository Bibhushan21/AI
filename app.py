...
def mcp_fetch():
    try:
        ...
        data_source = data.get('data_source', 'all')  # New parameter for data source selection
        app.logger.info(f'Received query: {query}, fetch_only: {fetch_only}, data_source: {data_source}')
        
        # Check cache first
        cache_key = get_cache_key('fetch', {'query': query, 'fetch_only': fetch_only, 'data_source': data_source})
        ...
        if fetch_only:
            result = asyncio.run(master.fetch_data_only(params, data_source))
        else:
            result = asyncio.run(master.fetch_all_data(params, data_source))
